  ...
  if (data) {
    ...
    const royalty = data
      .slice(
        8 + // key
          ...
          8 + //price
          4 +
          MAX_SYMBOL_LENGTH, // u32 len + symbol //max supply
        8 + // key
          32 + // authority
          32 + //wallet
          tokenMintSpace + // token mint
    ...
    if (creatorsLength) {
      ...
      for (let index = 0; index < creatorsLength; index++) {
        ...
        const creatorShare =
          data
            .slice(creatorOffset + 33, creatorOffset + 34)
            .readUInt8()
            .toString() + "%";
        creators.push({
          creatorAddress: bs58.encode(creatorPubKey),
    ...
    const whiteListSpace = whiteListOption
      ? data
          .slice(
            8 + // key
              32 + // authority
        ...
        ? 44
        : 36
      : 1; //discount price is 8 bytes and the 32 bytes is from the pubkey
    ...
    for (let index = 0; index < itemsAvailable; index++) {
      ...
      configOffset += 36;
      const nftUri = data
        .slice(configOffset + 4, configOffset + 200)
        .toString();
      configOffset += 204;
